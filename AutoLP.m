...
include_SCC = 1;   % binary variable for whether or not to include social cost of carbon in the objective function; 1 = include, 0 = exclude
...
%     SCC = xlsread(input_filename,'SCC');   %Social cost of carbon in $/ton of CO2
    ...
    for k=1:analysis_horizon
        fLDV_SCC = fLDV_SCC + (SCC(k)*(1.03/(1+discountrate))^(k-1))*AemLDV(k,:);    % 1.03 factor introduced because a 3% discount rate is built into the SCC values from literature
        fLDV_SCC_yearly(k,:) = (SCC(k)*(1.03/(1+discountrate))^(k-1))*AemLDV(k,:);
        fLDV_SCC_const_yearly(k) = (SCC(k)*(1.03/(1+discountrate))^(k-1))*BemLDV(k);
    end
