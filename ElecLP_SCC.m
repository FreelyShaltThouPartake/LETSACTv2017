    ...
    for cay=1:caylength
        ...
        if cay==4
            ...
            while SCC_flag > 0
                ...
                for k=1:analysis_horizon
                    fUtil_SCC = fUtil_SCC + (SCC*((1+SCC_inc_rate)^(k-1))/(1+discountrate)^(k-1))*AemUtil(k,:);    % 1.03 factor introduced because a 3% discount rate is built into the SCC values from literature
                    fUtil_SCC_yearly(k,:) = (SCC*((1+SCC_inc_rate)^(k-1))/(1+discountrate)^(k-1))*AemUtil(k,:);
                    fUtil_SCC_const_yearly(k) = (SCC*((1+SCC_inc_rate)^(k-1))/(1+discountrate)^(k-1))*BemUtil(k);
                end
        ...
        lbUtilmod(1:N_Util*cay) = 0.95*xbau(1:N_Util*cay);
        lbUtilmod(N_Util*analysis_horizon + 1:N_Util*analysis_horizon + N_Util*cay*Tcrit_Util) = 0.95*xbau(N_Util*analysis_horizon + 1:N_Util*analysis_horizon + N_Util*cay*Tcrit_Util);
        ubUtilmod(1:N_Util*cay) = 1.05*xbau(1:N_Util*cay);
        ubUtilmod(N_Util*analysis_horizon + 1:N_Util*analysis_horizon + N_Util*cay*Tcrit_Util) = 1.05*xbau(N_Util*analysis_horizon + 1:N_Util*analysis_horizon + N_Util*cay*Tcrit_Util);
